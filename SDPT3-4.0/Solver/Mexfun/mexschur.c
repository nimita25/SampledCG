...
*
********************************************************/
void setvec(mwSize n, double *x, double alpha)
...
**********************************************************/
void schurij1(mwSize n, double *Avec, 
              mwIndex *idxstart, mwIndex *nzlistAi, mwIndex *nzlistAj,
              double *U, mwSize col, double *schurcol)
...
**********************************************************/
void schurij3(mwSize n,  double *Avec, 
              mwIndex *idxstart, mwIndex *nzlistAi, mwIndex *nzlistAj,
              double *U, double *V, mwSize col, double *schurcol)
...
**********************************************************/
void vec(mwSize numblk, mwIndex *cumblksize, mwIndex *blknnz, 
...
void schurij2(double *Avec, 
              ...
              mwIndex *blkidx, mwSize col, double *schurcol)
...
{    mxArray  *blk_cell_pr;  
     ...
     mwIndex  subs[2];
     mwSize   nsubs=2; 
     mwSize   index, colend, type, isspU, isspV, numblk, nzP, existP; 
     mwSize   len, row, col, nU, nV, n, m, m1, idx1, idx2, l, k, nsub, n1, n2, opt, opt2;
     mwSize   kstart, kend, rb, cb, cblk, colcb, count; 
     double   tmp; 
...
/**** get pointers ****/    
    ...
    for (k=0; k<len; k++) { 
        idxstart[k] = (mwSize)idxstarttmp[k]; 
    }
    ...
    for (k=0; k<len; k++) { 
        nzlistAi[k] = (mwSize)nzlistAtmp[k] -1; /* -1 to adjust for matlab index */
        nzlistAj[k] = (mwSize)nzlistAtmp[k+len] -1; 
    }
    permAtmp = mxGetPr(prhs[4]); 
    m1 = mxGetN(prhs[4]); 
    permA = mxCalloc(m1,sizeof(mwIndex)); 
    for (k=0; k<m1; k++) {
        permA[k] = (mwSize)permAtmp[k]-1; /* -1 to adjust for matlab index */
    }
    ...
    colend = (mwSize)*mxGetPr(prhs[7]); 
    type   = (mwSize)*mxGetPr(prhs[8]); 
...
for (col=0; col<colend; col++) { 
    ...
    for (row=0; row<=col; row++) {
	    if (schurcol[row] != 0) {
	       if (count<nzP & nlhs==3) { jcP[col+1]=count+1; irP[count]=row; P[count]=1; }
	       count++; 
